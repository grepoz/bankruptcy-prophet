...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Command line parameters for training.')

    parser.add_argument("-d", "--dataset_name", type=str, default='longer_moviereview',
                        help="directory of data encoded by token-level Roberta")
    ...
    parser.add_argument("-m", "--max_len", type=int, default=16,
                        help="the maximum sentences number per document")
    parser.add_argument('-r', '--random_seeds', nargs='+', default=['2024'],
                        help="List of random seeds")
    parser.add_argument('-s', '--training_size', nargs='+', default=['20'],
    ...
    for number in options.training_size:
        if int(number) > 200:
            parser.error(
                "The largest training size is 200, you can customize the maximum training size by modifying the corrsponding codes of initializing training set.")
    ...
    for train_size in train_sizes:
        ...
        for seed in random_states:
            ...
            for e in tqdm(range(no_epochs)):
                ...
                for i, data in enumerate(tqdm(trainloader)):
                    ...
                    with torch.no_grad():
                        ...
                        train_tn, train_fp, train_fn, train_tp = confusion_matrix(y_train_true, y_train_pred,
                                                                                  labels=[0, 1]).ravel()

                        train_fpr, train_tpr, train_thresholds = metrics.roc_curve(y_train_true, y_train_prob_pos,
                                                                                   pos_label=1)
                        train_step_auc = metrics.auc(train_fpr, train_tpr)
                        train_aucs.append(train_step_auc)

                        train_confusion_matrices.append(
                            {'TP': train_tp, 'TN': train_tn, 'FP': train_fp, 'FN': train_fn})

                        print('Epoch: ', e, ' step:', i, ' training accuracy: ', train_acc)
                        print('Epoch: ', e, ' step:', i, ' training AUC:', train_step_auc)
                        print('Epoch: ', e, ' step:', i, ' training confusion matrix:', 'TP', train_tp, 'TN', train_tn,
                        ...
                        if config.output_attentions:
                            ...
                            if best_auc < train_step_auc:
                                ...
                                torch.save(attention_scores,
                                           attention_output_dirs + 'epoch%s_seed%s_size%s.pt' % (e, seed, train_size))
            ...
            df.to_csv(os.path.join(args['verbose_output_dir'], 'all_raw_%s_h%s_s%s_size%s.csv' % (
                dataset, config.num_attention_heads, seed, train_size)), index=False)

            df_auc = pd.DataFrame()
            df_auc[seed] = [row for row in aucs]
            df_auc.to_csv(os.path.join(args['verbose_output_dir'], 'all_auc_%s_h%s_s%s_size%s.csv' % (
                dataset, config.num_attention_heads, seed, train_size)), index=False)

            df_loss = pd.DataFrame()
            df_loss[seed] = [row for row in losses]
            df_loss.to_csv(os.path.join(args['verbose_output_dir'], 'all_loss_%s_h%s_s%s_size%s.csv' % (
                dataset, config.num_attention_heads, seed, train_size)), index=False)

            df = pd.DataFrame()
            df[seed] = [row for row in train_confusion_matrices]
            df.to_csv(os.path.join(args['verbose_output_dir'], 'all_train_raw_%s_h%s_s%s_size%s.csv' % (
                dataset, config.num_attention_heads, seed, train_size)), index=False)

            df_auc = pd.DataFrame()
            df_auc[seed] = [row for row in train_aucs]
            df_auc.to_csv(os.path.join(args['verbose_output_dir'], 'all_train_auc_%s_h%s_s%s_size%s.csv' % (
                dataset, config.num_attention_heads, seed, train_size)), index=False)
